...
contract DirectGrantsLiteStrategy is Native, BaseStrategy, Multicall {
	...
	function initialize(
		uint256 _poolId,
		bytes memory _data
	) external virtual override onlyAllo {
	...
	function __DirectGrantsLiteStrategy_init(
		uint256 _poolId,
		InitializeData memory _initializeData
	) internal {
	...
	) external view returns (Recipient memory recipient) {
	...
	function _registerRecipient(
		...
		recipient.useRegistryAnchor = useRegistryAnchor
			? true
		...
		if (recipientToStatusIndexes[recipientId] == 0) {
			...
			if (
				currentStatus == uint8(Status.Accepted) ||
				currentStatus == uint8(Status.InReview)
			) {
	...
	function _allocate(
		...
		for (uint256 i = 0; i < length; ) {
			...
			_transferAmountFrom(
				token,
				TransferData({from: _sender, to: recipientAddress, amount: amount})
			);
	...
	function _getRecipient(
		address _recipientId
	) internal view returns (Recipient memory) {
	...
	function _setRecipientStatus(address _recipientId, uint256 _status) internal {
		...
		(
			...
			uint256 colIndex,
			uint256 currentRow
		) = _getStatusRowColumn(_recipientId);
