	...
	Errors
{
	...
	function registerRecipient(
		...
		return
			pools[_poolId].strategy.registerRecipient{value: msg.value}(
				_data,
				msg.sender
			);
	}
	...
	function batchRegisterRecipient(
		...
		for (uint256 i; i < poolIdLength; ) {
			recipientIds[i] = pools[_poolIds[i]].strategy.registerRecipient(
				_data[i],
				msg.sender
			);
			unchecked {
	...
	function _createPool(
		...
		if (
			_strategy.getPoolId() != poolId ||
			address(_strategy.getAllo()) != address(this)
		) revert MISMATCH();
	...
	function _fundPool(
		...
		if (_token == NATIVE) {
			...
			uint256 balanceBeforeFundingPool = _getBalance(
				_token,
				address(_strategy)
			);
			_transferAmountFrom(
	...
	function _isPoolManager(
		uint256 _poolId,
		address _address
	) internal view returns (bool) {
		return
			hasRole(pools[_poolId].managerRole, _address) ||
			_isPoolAdmin(_poolId, _address);
	}
